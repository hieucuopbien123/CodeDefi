...
contract TestUniswapOptimal {
    ...
    function sqrt(uint y) internal pure returns (uint z) {//lấy hàm sqrt vì trong công thức s có
        if (y > 3) {
            z = y;
            uint x = y / 2 + 1;
        while (x < z) {
    ...
    */
    ...
    }//tính lượng tokenA cần cho vào pool để nhận lại tokenB sao cho tỉ lệ tokenA:tokenB bằng với tỉ lệ đó trong pool
    //sau khi thực hiện swap
    ...
    */
    ...
    /* sub-optimal one-sided supply -> tức Th k optimal. Ta sẽ test cả 2 hàm
